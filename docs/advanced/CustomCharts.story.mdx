...
an area chart, bar chart and scatter chart. This chart is more than just pretty, it served as a central element
for cyber-security analysts to identify trends and threats. For more information on the chart and design
...
by the amazing designer [Harlan Elam](https://dribbble.com/harlanelam). The chart rebuilt and modified slightly from
Harlan's original design for legal reasons.
...
While the chart looks pretty intense, its actually pretty simple as you can see.

Let's take a minute to walk through the [code](https://github.com/reaviz/reaviz/blob/master/docs/advanced/Radar.tsx) for this chart now
we understand the general structure of the chart.

The chart starts with the `ChartContainer` component. This component deals with measuring sizes and creating the root
svg element for us.
...
Inside the `ChartContainer` component, we have a callback that gives
...
These function is mainly leveraging the `d3-scale` library to build the scales
