...
import { BarSeries, BarSeriesProps } from '../BarChart/BarSeries';
import {
  ChartDataShape,
...
import css from '../BarChart/BarChart.module.css';
import {
  ChartContainer,
...
export const BarChart: FC<Partial<BarChartProps>> = ({
  ...
  */
  ...
  const gridlines = React.Children.only(GridlineSeries);
  const brush = React.Children.only(ChartBrush);
  const series = React.Children.only(BarSeries);
  // TODO: figure out how to get secondary
  const xAxis = React.Children.only(LinearXAxis);
  const yAxis = React.Children.only(LinearYAxis);
